...
import { getBarangayList, getCityList, getProvinceList } from "../actions";
...
const AddressCard = ({ index, onRemove, onSetDefault, isDefault, isOnly, regionList }: Props) => {
  ...
  const resetOptions = (level: number, value: string | null) => {
    ...
    if (value) {
      setLoadingFieldList(loadingList);
    }
  };
  ...
  const handleRegionChange = async (value: string) => {
    ...
    try {
      ...
      setValue(`addresses.${index}.provinceOptions`, provinceData);
    } catch (e) {
      notifications.show({
  ...
  const handleProvinceChange = async (value: string) => {
    ...
    try {
      ...
      setValue(`addresses.${index}.cityOptions`, cityData);
    } catch (e) {
      notifications.show({
        message: "Something went wrong. Please try again later.",
        color: "red",
  ...
  const handleCityChange = async (value: string) => {
    ...
    try {
      ...
      setValue(`addresses.${index}.barangayOptions`, barangayData);
    } catch (e) {
      notifications.show({
        message: "Something went wrong. Please try again later.",
        color: "red",
  ...
  const handleBarangayChange = async (value: string) => {
    if (!userData) return;
    try {
      const barangayList = getValues(`addresses.${index}.barangayOptions`);
      const postalCode = barangayList.find((barangay) => barangay.value === value)?.postalCode;
      if (!postalCode) throw new Error(`Missing postal code ${value}`);

      setValue(`addresses.${index}.postalCode`, postalCode);
    } catch (e) {
      notifications.show({
        message: "Something went wrong. Please try again later.",
        color: "red",
